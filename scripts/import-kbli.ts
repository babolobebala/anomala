...
function readArguments(): { filePath: string, apply: boolean } {
  ...
  if (!filePath) {
    throw new KbliImportFileError('Usage: pnpm import:kbli -- "path/to/file.xlsx" [--apply]')
  }
...
async function main(): Promise<void> {
  ...
  try {
    const result = await importKbliWorkbook({
      database: prisma as unknown as KbliImportDatabase,
      filePath,
      apply
    })
...
main().catch((error) => {
  const message = error instanceof Error ? error.message : 'KBLI import failed.'
