...
import type { KbliFilterOptions, KbliFilterState } from '~/types/kbli'
...
type RegionFilterField = 'kecamatan' | 'desa' | 'namaSls' | 'ppl' | 'pml'
...
const props = defineProps<{
  filters: KbliFilterState
  search: string
  totalAssignments: number
}>()

const emit = defineEmits<{
  'update:filters': [filters: KbliFilterState]
  'update:search': [search: string]
}>()

const drawerOpen = ref(false)
const draftRegionFilters = reactive<Record<RegionFilterField, string>>({
  kecamatan: '',
  desa: '',
  namaSls: '',
  ppl: '',
...
const { data: regionOptions, status: regionOptionsStatus }
  = await useFetch<KbliFilterOptions>('/api/kbli/filter-options', {
    query: regionOptionsQuery
  })

const handlingOptions: SelectItem[] = [
  { label: 'Semua status', value: ALL_VALUE },
  { label: 'Belum selesai', value: 'unhandled' },
  { label: 'Selesai (tandai)', value: 'handled' }
]

const kategoriOptions = computed<SelectItem[]>(() => [
  { label: 'Semua kategori', value: ALL_VALUE },
  ...(regionOptions.value?.kategori ?? [])
    .map(item => ({
      label: item.deskripsi
        ? `${item.kode.trim()} — ${item.deskripsi}`
        : item.kode.trim(),
...
const selectedHandlingLabel = computed(
  () =>
    handlingOptions.find(
      option => option.value === toSelectValue(props.filters.completionStatus)
...
function updateQuickFilter(field: 'kategori' | 'completionStatus', value: string): void {
  emit('update:filters', {
    ...props.filters,
...
<template>
  ...
  >
    <div class="filter-toolbar__summary">
      <USelectMenu
        :model-value="toSelectValue(filters.completionStatus)"
    ...
    <div class="filter-toolbar__controls">
      ...
      <USelectMenu
        :model-value="toSelectValue(filters.kategori)"
        ...
        @update:model-value="
          updateQuickFilter('kategori', String($event ?? ''))
        "
      />
