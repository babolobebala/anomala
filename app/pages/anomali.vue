...
const { data: executors } = await useFetch<ExecutorOption[]>('/api/anomali/executors')
...
function isOperationallyResolved(anomaly: AnomalyListItem): boolean {
  return anomaly.isHandled || anomaly.isSesuaiLapangan
}

function recalculateGroupSummary(group: AssignmentAnomalyGroup): void {
...
async function toggleFieldCondition(
  group: AssignmentAnomalyGroup,
  anomaly: AnomalyListItem
): Promise<void> {
  if (!anomaly.isActive || saving[anomaly.id]) {
  ...
  anomaly.sesuaiLapanganAt = nextIsSesuaiLapangan ? new Date().toISOString() : null
  recalculateGroupSummary(group)

  try {
    const result = await $fetch<FieldConditionResponse>(
      `/api/anomali/${anomaly.id}/field-condition`,
      {
        method: 'PATCH',
    ...
    Object.assign(group.summary, previous.summary)
    mutationError.value
      = 'Status kondisi lapangan gagal diperbarui. Perubahan lokal dikembalikan.'
  } finally {
    saving[anomaly.id] = false
  }
}
...
async function updateAssignmentExecutor(
  ...
  try {
    const result = await $fetch<AssignmentExecutorResponse>(
      `/api/anomali/assignment/${encodeURIComponent(group.assignmentId)}/executor`,
      {
        method: 'PATCH',
        body: { eksekutorId }
      }
          ...
          <tbody :aria-label="isListSkeletonVisible ? 'Memuat data anomali' : undefined">
            ...
            <template v-else>
              <AnomaliAssignmentGroup
                ...
                @toggle-field-condition="toggleFieldCondition(group, $event)"
